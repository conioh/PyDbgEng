...
class PyDbgEng(IDebugEventCallbacksSink):
    ...
    def __init__(self, event_callbacks_sink = None, output_callbacks_sink = None, dbg_eng_dll_path = None, symbols_path = None):
        ...
        if (event_callbacks_sink != None):
            ...
            self.event_proxy_creator = CreateObject("DbgEventProxy.EventCallbacksProxy.1", sink=self, sourceinterface=DBGEVENTPROXYLib._IEventCallbacksProxyEvents)
            event_interest_mask = self.GetInterestMask()
            self.dbg_eng_log("PyDbgEng.__init__: got event_interest_mask %x" % event_interest_mask)
            event_proxy_iunknown = self.event_proxy_creator.CreateIDebugEventCallbacks(event_interest_mask)
        ...
        if (output_callbacks_sink != None):
            ...
            self.output_proxy_creator = CreateObject("DbgEventProxy.OutputCallbacksProxy.1", sink=output_callbacks_sink, sourceinterface=DBGEVENTPROXYLib._IOutputCallbacksProxyEvents)
            output_proxy_iunknown = self.output_proxy_creator.CreateIDebugOutputCallbacks()
            output_proxy = cast(output_proxy_iunknown, POINTER(DbgEng.IDebugOutputCallbacks))
            self.old_output_callbacks = self.idebug_client.GetOutputCallbacks()
    ...
    def Breakpoint(self, this, Offset, Id, BreakType, ProcType, Flags, DataSize, DataAccessType, PassCount, CurrentPassCount, MatchThread, CommandSize, OffsetExpressionSize):
        if (self.breakpoints.has_key(Id)):
            handler = self.breakpoints.get(Id)
            handler(self)
            
        if (self.event_callbacks_sink_intereset_mask & DbgEng.DEBUG_EVENT_BREAKPOINT):
            ret = self.event_callbacks_sink.Breakpoint(self, Offset, Id, BreakType, ProcType, Flags, DataSize, DataAccessType, PassCount, CurrentPassCount, MatchThread, CommandSize, OffsetExpressionSize)
            if (ret != None):
                return ret
    ...
    def Exception(self, this, ExceptionCode, ExceptionFlags, ExceptionRecord, ExceptionAddress, NumberParameters, ExceptionInformation0, ExceptionInformation1, ExceptionInformation2, ExceptionInformation3, ExceptionInformation4, ExceptionInformation5, ExceptionInformation6, ExceptionInformation7, ExceptionInformation8, ExceptionInformation9, ExceptionInformation10, ExceptionInformation11, ExceptionInformation12, ExceptionInformation13, ExceptionInformation14, FirstChance):
        if (self.event_callbacks_sink_intereset_mask & DbgEng.DEBUG_EVENT_EXCEPTION):
            ret = self.event_callbacks_sink.Exception(self, ExceptionCode, ExceptionFlags, ExceptionRecord, ExceptionAddress, NumberParameters, ExceptionInformation0, ExceptionInformation1, ExceptionInformation2, ExceptionInformation3, ExceptionInformation4, ExceptionInformation5, ExceptionInformation6, ExceptionInformation7, ExceptionInformation8, ExceptionInformation9, ExceptionInformation10, ExceptionInformation11, ExceptionInformation12, ExceptionInformation13, ExceptionInformation14, FirstChance)
            if (ret != None):
                return ret
    ...
    def CreateProcess(self, this, ImageFileHandle, Handle, BaseOffset, ModuleSize, ModuleName, ImageName, CheckSum, TimeDateStamp, InitialThreadHandle, ThreadDataOffset, StartOffset):
        if (self.event_callbacks_sink_intereset_mask & DbgEng.DEBUG_EVENT_CREATE_PROCESS):
            ret = self.event_callbacks_sink.CreateProcess(self, ImageFileHandle, Handle, BaseOffset, ModuleSize, ModuleName, ImageName, CheckSum, TimeDateStamp, InitialThreadHandle, ThreadDataOffset, StartOffset)
            if (ret != None):
                return ret
